...
def topup_methods_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("💳 الدفع عبر NOWPayments", callback_data="topup_method:nowpayments")],
...
def get_nowpayments_api_key() -> str:
    return os.getenv(NOWPAYMENTS_API_KEY_ENV_NAME, "").strip()


def get_nowpayments_jwt_token() -> str:
    return os.getenv(NOWPAYMENTS_JWT_TOKEN_ENV_NAME, "").strip()


def get_nowpayments_email() -> str:
    return os.getenv(NOWPAYMENTS_EMAIL_ENV_NAME, "").strip()


def get_nowpayments_password() -> str:
    return os.getenv(NOWPAYMENTS_PASSWORD_ENV_NAME, "").strip()
